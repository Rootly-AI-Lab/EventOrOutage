...
class NaiveAnomalyDetecter:
    ...
    def get_anomalies(self, df):
        ...
        for website in df['website'].unique():
            ...
            for geo in website_data['geo'].unique():
                # Create a copy of the filtered data to avoid SettingWithCopyWarning
                geo_data = website_data[website_data['geo'] == geo].copy()
                ...
                if len(anomalous_dates) > 0:
                    if website not in results:
                        results[website] = {}
                    if geo not in results[website]:
                        results[website][geo] = []
                    results[website][geo].extend(anomalous_dates)
        return results
